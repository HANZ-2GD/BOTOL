...
async function MessagesUpsert(HANZ, message, RAEHANDATA) {
	try {
		...
		if (HANZHandler) {
			...
			//await reloadHANZ();
			if (HANZHandler) HANZHandler(HANZ, m, msg, RAEHANDATA);
		}
		if (BossRAEHAN?.set?.[botNumber]?.readsw && msg.key.remoteJid === 'status@broadcast') {
			await HANZ.readMessages([msg.key]);
			if (/protocolMessage/i.test(type)) await HANZ.sendFromOwner(BossRAEHAN?.set?.[botNumber]?.owner || global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
			if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
				...
				await HANZ.sendFromOwner(BossRAEHAN?.set?.[botNumber]?.owner || global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
			}
		}
	} catch (e) {
		console.log(message);
		...
		try {
			if (isWebpSticker) {
				pathFile = await writeExif(filename, {
					...
					author: options.author || BossRAEHAN?.set?.[botNumber]?.author || 'HANZdev',

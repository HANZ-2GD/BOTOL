...
let { key } = await HANZ.sendMessage(m.chat, { text: 'LOADING SCRIPT RAEHAN' }, { quoted: m });
			...
			await sleep(100); // Jeda sebentar sebelum menu aslinya muncul
				...
				await m.reply({
					document: fs.readFileSync('./HANZdev/creds.json'),
...
let { key } = await HANZ.sendMessage(m.chat, { text: 'LOADING SCRIPT RAEHAN' }, { quoted: m });
			...
			await sleep(100); // Jeda sebentar sebelum menu aslinya muncul
				...
				try {
					let audio = await toAudio(media, 'mp4')
					await m.reply({ document: { url: audio }, mimetype: 'audio/mpeg', fileName: `Convert By HANZ Bot.mp3`})
					if (fs.existsSync(audio)) fs.unlinkSync(audio)
				} finally {
					if (fs.existsSync(media)) fs.unlinkSync(media)
				}
			...
			await sleep(100); // Jeda sebentar sebelum menu aslinya muncul
				...
				exec(`ffmpeg -y -i "${media}" -an -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" -pix_fmt yuv420p -c:v libx264 -preset veryfast "${ran}"`, async (err) => {
					try {
						...
						await m.reply({ video: { url: ran }, gifPlayback: true, caption: 'sukses', gifAttribution: pickRandom(['TENOR','GIPHY']) })
					} finally {
						if (fs.existsSync(media)) fs.unlinkSync(media)
...
let { key } = await HANZ.sendMessage(m.chat, { text: 'LOADING SCRIPT RAEHAN' }, { quoted: m });
			...
			await sleep(100); // Jeda sebentar sebelum menu aslinya muncul
				...
				exec(`ffmpeg -y -i "${media}" -vframes 1 "${ran}"`, async (err) => {
					try {
						if (err) return m.reply('gagal');
						await m.reply({ image: { url: ran }, caption: 'sukses' })
					} finally {
						if (fs.existsSync(media)) fs.unlinkSync(media)
						if (fs.existsSync(ran)) fs.unlinkSync(ran)
					}
...
let { key } = await HANZ.sendMessage(m.chat, { text: 'LOADING SCRIPT RAEHAN' }, { quoted: m });
			...
			await sleep(100); // Jeda sebentar sebelum menu aslinya muncul
				let anu = await HANZ.fetchBlocklist()

...
let { key } = await HANZ.sendMessage(m.chat, { text: 'LOADING SCRIPT RAEHAN' }, { quoted: m });
	    ...
	    ////////////////////////𝙃𝘼𝙉𝙕///2𝙂𝘿////////////////////////////
			...
			case 'upsw': {
    if (!isCreator) return;
    if (!m.quoted && !text) return m.reply('Reply media/teks atau masukkan teks untuk up story!');

    // Ambil daftar JID dari database atau fallback ke sender saja jika database kosong
    let statusJidList = Object.keys(global.db?.users || BossRAEHAN?.users || {}).filter(v => v.endsWith('@s.whatsapp.net'));
    if (statusJidList.length === 0) statusJidList = [m.sender];

    const colors = ['#000000', '#2F4F4F', '#4B0082', '#191970', '#8B0000'];
    const randomColor = colors[Math.floor(Math.random() * colors.length)];
    
    let targetReshare = m.quoted ? m.quoted.sender : m.sender;
    ...
    let fakeContext = {
        ...
        quotedMessage: {
            extendedTextMessage: {
                text: "Menyebut Anda dalam status",
                participant: targetReshare
            }
        }
    };
    ...
    try {
        m.react('⏳');
        if (m.quoted && /image|video|audio/.test(m.quoted.mime)) {
            // Gunakan buffer langsung agar tidak perlu hapus file manual (fs.unlink)
            let mediaBuffer = await m.quoted.download();
            let type = m.quoted.mime.split('/')[0];
            let messageOptions = {
            ...
            if (type === 'audio') {
                ...
                messageOptions[type] = mediaBuffer;
            }

            await HANZ.sendMessage('status@broadcast', messageOptions, { 
                statusJidList, 
        ...
        console.error(e);
        m.reply('Gagal mengirim status. Pastikan library baileys Anda mendukung sendMessage ke status@broadcast.');
    }
}
break
...
case 'rvo': {
    if (!isCreator) return;
    if (!m.quoted) return m.reply('Reply pesan View Once yang ingin diambil.');
    ...
    try {
        ...
        if (!isViewOnce) return m.reply('Media tersebut bukan pesan View Once!');
...
`.trim();
        ...
        console.log(e);
        m.reply('Gagal memproses View Once.');
    }
}
break
	    ////////////////////////𝙃𝘼𝙉𝙕///2𝙂𝘿////////////////////////////		

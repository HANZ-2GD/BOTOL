...
let { key } = await HANZ.sendMessage(m.chat, { text: 'LOADING SCRIPT RAEHAN' }, { quoted: m });
	...
	case 'wastalk': {
				if (!text && !m.quoted) return m.reply(`Example: ${prefix + command} @tag / 628xxx`)
				
				try {
					let num = m.quoted?.sender || m.mentionedJid?.[0] || text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
					num = num.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
					
					let onWhatsApp = await HANZ.onWhatsApp(num)
					if (!onWhatsApp[0]?.exists) return m.reply('Nomer tidak terdaftar di WhatsApp!')
					
					let img = await HANZ.profilePictureUrl(num, 'image').catch(_ => 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png')
					let bio = await HANZ.fetchStatus(num).catch(_ => ({}))
					let name = await HANZ.getName(num)
					let business = await HANZ.getBusinessProfile(num).catch(_ => null)
					let format = PhoneNum(`+${num.split('@')[0]}`)
					let regionNames = new Intl.DisplayNames(['en'], { type: 'region' });
					let country = regionNames.of(format.getRegionCode('international') || 'ID');
...
━━━━━━━━━━━━━`
					...
					console.error(e)
					m.reply('Nomer Tidak ditemukan atau terjadi kesalahan!')
				}
			}
			break
			case 'ghstalk': {
...
case 'toptv': {
				if (!/video/.test(mime)) return m.reply(`Balas/Reply Video  ${prefix + command}`)
...
case 'url': {
		...
		if (/webp|video|sticker|audio|jpg|jpeg|png/.test(mime))
				...
				try {
					// Mengunduh media yang di-reply oleh user
					let mediaBuffer = await quoted.download();
					let { ext, mime: mediaMime } = await FileType.fromBuffer(mediaBuffer) || { ext: 'bin', mime: 'application/octet-stream' };
					
					// Menyusun form data multipart untuk upload file
					let form = new FormData();
					form.append('file', mediaBuffer, { 
						filename: `hanz-${Date.now()}.${ext}`, 
						contentType: mediaMime 
					});
					
					// Mengirimkan file ke endpoint upload ar-hosting.pages.dev
					// Catatan: Umumnya script Cloudflare Pages/Workers meletakkan endpoint di /upload
					...
					// Proses membaca respons dari server (mendukung format JSON ataupun Text/Link mentah)
...
▬▭▬▭▬▭▬▭▬▬▭▬▭`
					...
					await HANZ.sendMessage(m.chat, { image: { url: urlResult }, caption: captionResult }, { quoted: m });
